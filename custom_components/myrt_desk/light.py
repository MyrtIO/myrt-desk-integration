...
# Map enum to string list
EFFECT_LIST = [effect.name.capitalize() for effect in LightEffect]
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            if ATTR_EFFECT in kwargs:
                ...
                effect_index = EFFECT_LIST.index(kwargs[ATTR_EFFECT])
                if len(futures) > 0:
                    await gather(*futures)
                await self._api.set_effect(LightEffect(effect_index + 1))
                return
            if ATTR_BRIGHTNESS in kwargs:
                self._brightness = kwargs[ATTR_BRIGHTNESS]
                futures.append(
                    self._api.set_brightness(self._brightness))
